  ...
  <div class="person-modal-backdrop" role="dialog" aria-modal="true" :aria-label="`${person.name}演员资料`" @click.self="$emit('close')">
    <section class="person-modal">
      ...
      <template v-else>
        ...
        <section v-if="person.knownFor?.length"><h3>其他代表作品</h3><div class="known-for"><button v-for="work in person.knownFor" :key="work.id" type="button" :aria-label="`预览${work.title}`" @click="$emit('preview', work)"><img v-if="work.poster" :src="work.poster" :alt="work.title" /><div v-else></div><strong>{{ work.title }}</strong><span>{{ work.year }}</span></button></div></section>
      </template>
    </section>
  </div>
</template>
